          ...
          .then(
              resData => { 
                  ...
                  if(resData.success==1)      
                    ...
                    toast.error("Veeam Opeartion Failed");
              },
              error => {
                  toast.error("Veeam Opeartion Failed");
              }
...
function vmResize(formData){
    return dispatch => {
        ...
        vmlistService.vmResize(formData)      
            .then(
              resData => { 
                  dispatch(vmDetail(btoa(formData.user_id),btoa(formData.id)));
                  if(resData.success==1)      
                    toast.success(resData.message);
                  else
                    toast.error("VM Opeartion Failed");
...
function vmLogs(params) {
  ...
  function request(vmid) { return { type: vmlistConstants.VMLOGS_REQUEST, vmid } }
...
function vmDetail(clientid,vmid) {
  ...
  function request(vmid) { return { type: vmlistConstants.VMDETAIL_REQUEST, vmid } }
...
function getJobStatus(formData) {
  return dispatch => {
      dispatch(request());
      vmlistService.getJobStatus(formData)       
          .then(
            jobstatus => { 
                  dispatch(success(jobstatus));
              },
              error => {
                  dispatch(failure(error.toString()));
                  dispatch(alertActions.error(error.toString()));
  ...
  function request() { return { type: vmlistConstants.JOBDATA_REQUEST } }
  function success(jobstatus) { return { type: vmlistConstants.JOBDATA_SUCCESS, jobstatus } }
  function failure(error) { return { type: vmlistConstants.JOBDATA_FAILURE, error } }
}
