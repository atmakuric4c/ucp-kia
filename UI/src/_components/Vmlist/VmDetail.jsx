...
class VmDetail extends React.Component {
 ...
 updateVmList=()=>{
  ...
  vmOperations() {
    ...
    const postParams = { "ref_id": btoa(vmData.ref_id), "type": action,vm_id:btoa(vmData.id),user_id:btoa(this.state.user_id),clientid:btoa(this.state.clientid) };
  ...
  render() { 
    const { vmlist,veeamjob, profiles} = this.props;
    let loading=this.props.vmlist.loading;                                      
    ...
    return (
      <div className="container-fluid main-body">
        <div className="contentarea">
          <h5 className="color"><img src="/src/img/vmware_img.png" width="50px" className="mr-2" />VM Details</h5>
          {loading && <PageLoader/>}
          {vm_data &&  
          <React.Fragment>
          <div className="row vm-details">
            <div className="col-md-12 p-0 vm-details-row mt-3">
              <div className="col-md-12 mt-2 mb-2">
                {commonFns.fnCheckVMOperationsAuth(profiles, commonFns.vmOperations.Cloud4cOperations.OnOff) &&
                  <React.Fragment>
                  {
                    (vm_data.vm.vm_status.toLowerCase() == "poweredon" || vm_data.vm.vm_status.toLowerCase() == "running") ?
                      <span className="alert info-box-danger mr-2 cursor-pointer" onClick={() => this.vmAction(vm_data.vm,'2','Power Off')}><span className="fas fa-power-off mr-2"></span>Power Off</span>
                    : <span className="alert info-box-success mr-2 cursor-pointer" onClick={() => this.vmAction(vm_data.vm,'1','Power On')}><span className="fa fa-play mr-2"></span>Power On</span>
                  }
                  </React.Fragment>
                }

                {commonFns.fnCheckVMOperationsAuth(profiles, commonFns.vmOperations.Cloud4cOperations.Reboot) &&
                  <span className="alert info-box-success mr-2 cursor-pointer" onClick={() => this.vmAction(vm_data.vm,'4','Reboot')}><span className="fas fa-sync-alt mr-2"></span>Reboot</span>
                }

                {commonFns.fnCheckVMOperationsAuth(profiles, commonFns.vmOperations.Cloud4cOperations.Terminate) &&
                  <span className="alert info-box-danger mr-2 cursor-pointer" onClick={() => this.vmAction(vm_data.vm,'3','Terminate')}><span className="fa fa-times mr-2"></span>Terminate</span>
                }

                {commonFns.fnCheckVMOperationsAuth(profiles, commonFns.vmOperations.Cloud4cOperations.Resize) &&
                ...
                {commonFns.fnCheckVMOperationsAuth(profiles,commonFns.vmOperations.Cloud4cOperations.History) &&
            ...
            <div className="row p-0 clear-both">          
...
function mapStateToProps(state) {
  const { vmlist,vm_data,logData,veeamjob, common, profiles } = state;
  return {
    vmlist,
    vm_data,
    logData,
    veeamjob, common, profiles
  };
}

const connectedVmlist = connect(mapStateToProps)(VmDetail);
export { connectedVmlist as VmDetail };